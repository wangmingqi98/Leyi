...
public class HomeDetailCAdapter extends BaseAdapter {
    Context context;
    private List<InItBean.RecommandGongyingBean> recommand_gongying;
    int type;
    public HomeDetailCAdapter(Context context, List<InItBean.RecommandGongyingBean> recommand_gongying) {
    ...
    @Override
    public int getCount() {
        return recommand_gongying.size();
    }

    @Override
    public Object getItem(int position) {
        return recommand_gongying.get(position);
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.name.setText(recommand_gongying.get(position).getName());
        holder.rate_foramt.setText(recommand_gongying.get(position).getRate()+"%");
        holder.repay_time.setText(recommand_gongying.get(position).getRepay_time()+recommand_gongying.get(position).getRepay_time_type());
        holder.min_loan_money_fromat.setText(recommand_gongying.get(position).getMin_loan_money());
        holder.progress.setCurrentCount(Float.valueOf(recommand_gongying.get(position).getProgress_point()));
        holder.progress_point.setText(recommand_gongying.get(position).getProgress_point()+"%");
        holder.need_money.setText(recommand_gongying.get(position).getRemain_money());
        switch (recommand_gongying.get(position).getDeal_status()){
            case "1"://立即投资
        ...
        holder.now_repay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("type",recommand_gongying.get(position).getDeal_status());
                intent.putExtra("id",recommand_gongying.get(position).getDeal_id());
