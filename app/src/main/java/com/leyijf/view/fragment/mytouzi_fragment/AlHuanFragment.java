...
import com.leyijf.adapter.PayOffAdapter;
...
public class AlHuanFragment extends BaseFragment {
    public static final String TAG = "AlHuanFragment";
    ...
    private void initInvest() {
        ...
        observable.compose(compose(AlHuanFragment.this.bindToLifecycle())).subscribe(new BaseObserver<MyInvestBean>(getActivity()) {
            @Override
            protected void onHandleSuccess(MyInvestBean myInvestBean) {
                if(myInvestBean.getItem()!=null) {
                    ...
                    adapter = new PayOffAdapter(R.layout.mytouzi_item, item);
